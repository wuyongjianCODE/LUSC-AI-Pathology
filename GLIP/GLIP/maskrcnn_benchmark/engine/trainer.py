...
def do_train(
    ...
    all_queries, all_positive_map_label_to_token = create_queries_and_maps_from_dataset(data_loader.dataset,
                                                                                        cfg)
    if cfg.IMPROMPT.train_one_image_repeatly:
        saved_train_data=None
        start_iter=10
    if cfg.use_capt:
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("CAPromptLearner", self.model, self.optim, self.sched)
        ...
        if device_count > 1:
            ...
            model = CAPromptLearner(self.model)
    for iteration, (images, targets, idxs, positive_map, positive_map_eval, greenlight_map) in enumerate(data_loader, start_iter):
        if cfg.IMPROMPT.train_one_image_repeatly:
            if saved_train_data != None:
                (images, targets, idxs, positive_map, positive_map_eval, greenlight_map)=saved_train_data
            else:
                saved_train_data=(images, targets, idxs, positive_map, positive_map_eval, greenlight_map)
        # if idxs[0] not in [27]:
        #     labels=targets[0].extra_fields['labels']
        #     # if 1 in labels and 2 in labels and 3 not in labels and 4 not in labels and 5 not in labels :
        #     #     print(idxs[0])
        #     continue
        nnegative = sum(len(target) < 1 for target in targets)
        nsample = len(targets)
        # print('training img id: {}'.format(idxs))#img = Image.open(dataloader.dataset.coco.loadImgs(self.ids[idx])[0]['file_name']).convert('RGB')
        ...
        if cfg.SOLVER.USE_AMP:
            with autocast():
                if len(captions) > 0:
                    if cfg.print_flops:
                        from thop import profile
                        flops, params = profile(model, inputs=(images, targets))#, captions, positive_map,greenlight_map,all_positive_map_label_to_token))
                        print('FLOPs = ' + str(flops / 1000 ** 3) + 'G')
                        print('Params = ' + str(params / 1000 ** 2) + 'M')
                        start_time = time.time()
                    if cfg.MODEL.RPN_ONLY and cfg.MODEL.META_ARCHITECTURE!="GeneralizedRCNN":
                        loss_dict = model(images, targets, captions, positive_map, greenlight_map = greenlight_map,reference_map=all_positive_map_label_to_token,idx=idxs[0])
                    else:
        ...
        if enter_val_phase:
            ...
            if cfg.SOLVER.TEST_WITH_INFERENCE:
                with torch.no_grad():
                    ...
                    if cfg.USE_CAM:
                        for i, batch in enumerate(val_data_loader):
                            if cfg.USE_CAM and i > cfg.MAX_IMAGE_NUM_WHEN_USE_CAM:  # caution!!!it is too consuming to gen 5000, so only gen 300 image!!
                                continue
                            images, targets_, image_ids, positive_map, *_ = batch
                            with torch.no_grad():
                                images = images.to(device)
                                if positive_map is None:
                                    all_queries, all_positive_map_label_to_token = create_queries_and_maps_from_dataset(
                                        val_data_loader.dataset, cfg)
                                    output = model(images, captions=all_queries,positive_map=all_positive_map_label_to_token[0],reference_map=targets_)
                                else:
                                    captions = [t.get_field("caption") for t in targets_ if "caption" in t.fields()]
                                ...
                                if cfg.USE_CAM and i <= cfg.MAX_IMAGE_NUM_WHEN_USE_CAM:
                                    ...
                                    def fasterrcnn_reshape_transform(x):
                                        ...
                                        for value in x[0:1]:
                                            activations.append(
                                                torch.nn.functional.interpolate(torch.abs(value), target_size,
                                                                                mode='bilinear'))
                                    ...
                                    grayscale_cam = cam(input_tensor, targets=targets, targets0=None,
                                                        ...
                                                        positive_map=all_positive_map_label_to_token[0],reference_map=targets_)
                                    ...
                                    if 'fanew' in cfg.OUTPUT_DIR:
                                        ...
                                        savpath = 'PLOT2/' + cfg.OUTPUT_DIR + '/cam_of_iter{}'.format(iteration)
                                    if not os.path.exists(savpath):
                                        os.makedirs(savpath)
                                    io.imsave(savpath + '/' + val_data_loader.dataset.coco.imgs[id_to_img[image_ids[0]]][
                                        'file_name'], cam_image)
                                    ORIsavpath = 'ORI_WITH_BOX'
                                    if not os.path.exists(ORIsavpath):
                                        os.mkdir(ORIsavpath)
                                    ORIIM_PATH = ORIsavpath + '/' + \
                                                 val_data_loader.dataset.coco.imgs[id_to_img[image_ids[0]]]['file_name']
                                    if not os.path.exists(ORIIM_PATH):
                                        ...
                                        try:
                                            ...
                                            scores = [1 for ann in dataset['annotations'] if
                                                      ann['image_id'] == id_to_img[image_id]]
                                        cls_ids = [1 for ann in dataset['annotations'] if
                                                   ann['image_id'] == id_to_img[image_id]]
                ...
                for i, batch in enumerate(val_data_loader):
                    if cfg.USE_CAM and i>cfg.MAX_IMAGE_NUM_WHEN_USE_CAM and not cfg.VISUALIZE:#caution!!!it is too consuming to gen 5000, so only gen 300 image!!
                        continue
        ...
        if iteration % checkpoint_period == 0 and cfg.MODEL.SAVE_CKPT:
