...
class BoxList(object):
    ...
    def to(self, device):
        ...
        for k, v in self.extra_fields.items():
            if hasattr(v, "to"):
                v = v.to(device)
            bbox.add_field(k, v)
        if hasattr(self,'feat'):#wuyongjian:here we need to assign 'each bbox feature' into the Structure _proposals_,for further DEBUG
            bbox.feat=self.feat
        return bbox

    def __getitem__(self, item, restricted=False):
        bbox = BoxList(self.bbox[item], self.size, self.mode)
        for k, v in self.extra_fields.items():
            if not restricted:
                bbox.add_field(k, v[item])
            else:
                if k in ['labels','masks']:
                    bbox.add_field(k, v[item])
        return bbox
    ...
    def concate_box_list(list_of_boxes):
        ...
        if hasattr(list_of_boxes[0],'feat'):
            final.feat=list_of_boxes[0].feat
        return final
