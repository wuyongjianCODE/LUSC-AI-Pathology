...
class MaskRCNNLossComputation(object):
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            positive_inds = torch.nonzero(labels_per_image > -1).squeeze(1)
    ...
    def __call__(self, proposals, mask_logits, targets):
        ...
        #######################################################wuyongjian: only support classnum 1 and batchsize 1
        shift_train_patch=False
        if shift_train_patch:
            labels_this_image = proposals[0].extra_fields['labels']
            scores_this_image = proposals[0].extra_fields['scores']
            label_mask = (labels_this_image == 1)
            score_mask = (scores_this_image > 0.8)
            combined_mask = label_mask & score_mask
            positive_inds = torch.nonzero(combined_mask, as_tuple=True)[0]
        else:
        # positive_inds = torch.nonzero(labels_this_image == 1 and scores_this_image ==1).squeeze(1)#
        ...
        if self.vl_version:
            ...
            #     pred = torch.sigmoid(pred)  # 如果pred是logits
            #     intersection = (pred * target).sum()
            #     union = pred.sum() + target.sum()
            #     return 1 - (2 * intersection + smooth) / (union + smooth)
            # mask_loss = dice_loss(mask_logits_pos, mask_targets)
            # mse_loss = torch.nn.MSELoss(reduction='sum')
            # mask_loss = mse_loss(mask_logits_pos, mask_targets)
            mask_loss = F.binary_cross_entropy_with_logits(
                mask_logits_pos, mask_targets
            )
        else:
            mask_pred=mask_logits[positive_inds, labels_pos]
            mask_targets=mask_targets[positive_inds,:,:]
            # mse_loss = torch.nn.MSELoss(reduction='sum')
            # mask_loss = mse_loss(mask_pred, mask_targets)
            mask_loss = F.binary_cross_entropy_with_logits(
        ...
        if DEBUG:
            ...
            for i in range(10):
                ...
                plt.imshow(mask_logits_pos[k+i, :, :].detach().cpu().numpy())
                plt.subplot(2, 10, i+1+10)
                plt.imshow(mask_targets[k+i, :, :].detach().cpu().numpy())
            plt.show()
        return mask_loss
