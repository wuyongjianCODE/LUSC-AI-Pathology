...
class VLFuse(torch.nn.Module):
    ...
    def __init__(self, cfg, first_cross=False,skip_forward=False):
        ...
        if cfg.MODEL.DYHEAD.FUSE_CONFIG.TYPE == "MHA-S":
            ...
            if self.cfg.vpt_only<=-3 and first_cross:
                ...
                self.b_attn = BiAttentionBlockForCheckpoint(v_dim=self.joint_embedding_size,
                            ...
                            drop_path=.0,
                            init_values=1.0 / cfg.MODEL.DYHEAD.NUM_CONVS,
                            cfg=cfg,skip_forward=skip_forward
                            )
            if self.cfg.froze_fuse>0:
                for name, param in self.b_attn.named_parameters():
                    if 'adapter' not in name and 'cpt_' not in name:
    ...
    def init_configs(self, cfg):
        ...
        if self.lang_model in ["bert-base-uncased", "roberta-base", "clip"]:
            ...
            self.lang_dim = 1024
        # if cfg.IMPROMPT.gvl: #doesn't work , report error
        #     self.lang_dim=13343
    def forward(self, x):
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(cfg.MODEL.DYHEAD.NUM_CONVS):
            if cfg.MODEL.DYHEAD.FUSE_CONFIG.EARLY_FUSE_ON:
                ...
                if i in cfg.cpt_layers and cfg.vpt_only<=-3:
                    dyhead_tower.append(
                        VLFuse(cfg,first_cross=True)
                    )
                elif cfg.IMPROMPT.stage_num and (i not in cfg.IMPROMPT.stage_num):
                    dyhead_tower.append(
                        VLFuse(cfg,skip_forward=True)
                    )
                else:
                    dyhead_tower.append(
                        VLFuse(cfg)
    ...
    def forward(self, x, language_dict_features=None, embedding=None, swint_feature_c4=None,reference_feature=None):
        logits = []
        bbox_reg = []
        centerness = []
        ...
        if self.cfg.IMPROMPT.gvl==1:
            ...
            # dyhead_tower=feat_inputs
        elif self.cfg.IMPROMPT.gvl==2:
            dyhead_tower = self.dyhead_tower(feat_inputs)
            # dyhead_tower["visual"]=feat_inputs["visual"]
            # dyhead_tower["lang"]["hidden"]=feat_inputs["lang"]["hidden"]
        else:
            dyhead_tower = self.dyhead_tower(feat_inputs)
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
            ...
            if self.cfg.IMPROMPT.gvl2>0 and not self.training and reference_feature is not None:
                for feat_id,position in enumerate(reference_feature[-1]):
                    if reference_feature[feat_id] is not None:
                        dot_product_proj_tokens[0,position,:]=reference_feature[feat_id]
        ...
        for l, feature in enumerate(x):
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
                ...
                # ddsim=cos_similarity(dot_product_proj_tokens[0,:,:])
                # ori_after_sim=cos_similarity2(language_dict_features['embedded'],embedding)
                # ori_ori_sim = cos_similarity(language_dict_features['embedded'][0,:41,:])
                # aft_aft_sim = cos_similarity2(embedding, embedding)
                # if self.cfg.IMPROMPT.gvl==2:
                #     with open('example_box0.npy', 'rb') as f:
                #         example_box_feature0=numpy.load(f)
                #     with open('example_box1.npy', 'rb') as f:
                #         example_box_feature1=numpy.load(f)
                #     dot_product_proj_tokens[:,3,:]=torch.from_numpy(example_box_feature0)
                #     dot_product_proj_tokens[:,7, :] = torch.from_numpy(example_box_feature1)
                ...
                #     dotprod = dot_product_proj_tokens.cpu().numpy() #wuyongjian:convert to numpy to check value, wish to find the best matched line
                #     dpl=dot_product_logit.cpu().numpy()
                #     dp_max=numpy.argmax(dpl)
        ...
        if self.cfg.plot_tsne:
            fused_visual_features.append(dot_product_proj_tokens)
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.vpt_only==-2:
            self.vpt_input = nn.Parameter(torch.zeros(
                1, 256,7,7))
            nn.init.uniform_(self.vpt_input.data, -1, 1)
    def forward(self, images, features, targets=None,
        ...
        if self.cfg.vpt_only == -2:
            features[4]=features[4]+self.vpt_input
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_CONTRASTIVE_ALIGN_LOSS:
            # resizer needed
        ...
        if self.cfg.IMPROMPT.gvl>0 or self.cfg.IMPROMPT.gvl2>0:
    ...
    def _forward_train(self, box_cls, box_regression, centerness, targets, anchors,
        ...
        if self.cfg.MODEL.RPN_ONLY and not self.cfg.MODEL.RPN_NEED_RETURN_PROPOSALS:
    ...
    def _forward_test(self, box_regression, centerness, anchors,
        ...
        if self.cfg.IMPROMPT.gvl>0 or self.cfg.plot_tsne or self.cfg.append_locinfo_when_inference:
            boxes = self.box_selector_test(box_regression, centerness, anchors,
                                           box_cls,
                                           token_logits,
                                           dot_product_logits,
                                           positive_map,self.cfg
                                           )
            #TODO:wuyongjian,now need to refine boxes ,with fused_visual_features
            # fused_visual_features=features
            def get_box_feature_i_from_visual_feature(boxes,i,fused_visual_features):
                ...
                the_first_bbox_feature=the_first_bbox_stagefeature[:,:,int(the_first_bbox_loc)//int(stage_size_h),int(the_first_bbox_loc)%int(stage_size_h)]
            ...
            b_similary_to_range01=(b_similary-numpy.min(b_similary))/(numpy.max(b_similary)-numpy.min(b_similary))
            if False:
