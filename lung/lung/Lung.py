...
def y2lab(y):
    label=[]
    if y[0]>0.2:
        label.append('BASOPHIL')
    if y[1]>0.2:
        label.append('EOSINOPHIL')
    if y[2]>0.2:
        label.append('LYMPHOCYTE')
    if y[3]>0.2:
        label.append('MONOCYTE')
    if y[4]>0.2:
        label.append('NEUTROPHIL')
    print(label)
    return label
############################################################
#  Command Line
...
if __name__ == '__main__':
    ...
    # labs=np.array(labs)
    model0 = VGG16(weights='imagenet', include_top=False)
    x=model0.output
    ...
    NP = r'/data1/wyj/M/datasets/LUNG_4_CLASS/'
    NP_remained = r'/data1/wyj/M/datasets/LUNG_OD_4_CLASS_TESTSET/'
    ...
    if args.command == "train":
        for it in range(15):
            ...
            model.fit_generator(
                ...
                validation_data=validation_generator,
                validation_steps=10)
            model.save('resnet_lung_{}.h5'.format(it))
    ...
    if args.command == "detect":
        ...
        SVSDIR='/data1/wyj/M/datasets/data2/svs/'
        DETECT_NAME_ORI='828919-62022-04-25_16_25_04_x12969_y17434_373.png'
        for DIRPATH,subdir,fnames in os.walk(NP_remained):
            for fname in fnames:
                if '833736-72022-04-25_16_07_34' in fname:
                    DETECT_PATH=os.path.join(DIRPATH,fname)
                    DETECT_NAME=fname
                    SVSPATH=SVSDIR+DETECT_NAME[:DETECT_NAME.find('_x')]+'.svs'
                    slide = openslide.open_slide(SVSPATH)
                    [w, h] = slide.level_dimensions[0]
                    downscale = h / 2000
                    location_X=int(DETECT_NAME[DETECT_NAME.find('_x')+2:DETECT_NAME.find('_y')])/downscale
                    location_Y=int(DETECT_NAME[DETECT_NAME.find('_y')+2:DETECT_NAME.rfind('_')])/downscale
                    nailmap = slide.get_thumbnail((200000000,2000))
                    plt.imshow(nailmap)
                    im224=img_to_array(load_img(DETECT_PATH))
                    predr = model.predict(np.array([im224]))
                    pred_class=np.argmax(predr[0,:])
                    SWITCH=['cyan','lightgreen','red', 'blue',]
                    plt.gca().add_patch(plt.Rectangle(
                        xy=(location_X, location_Y),
                        width=(224)/downscale,
                        height=(224)/downscale,
                        edgecolor=SWITCH[pred_class],
