...
def inference(
    ...
    if cfg.eval_segm and cfg.MODEL.MASK_ON and cfg.MASK_ON_WHEN_TEST:
        iou_types = ("bbox","segm")
    if not task:
    ...
    if cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
        ...
        assert(0)
    # all_queries[0]='round purple cell. round purple nucleus. round deep purple cell. \
    #     round black cell.  oval purple cell. oval purple nucleus.\
    #     round black nucleus.'+all_queries[0]
    if cfg.input_nucleus_prompt or not cfg.SOLVER.TEST_WITH_INFERENCE:
        all_queries[0]='oval red cell.  oval red nucleus. round red cell.  round red nucleus. \
            oval purple cell.  oval purple nucleus. round purple cell.  round purple nucleus. \
            oval black cell.  oval black nucleus. round black cell.  round black nucleus. \
            oval deep purple cell.  oval deep purple nucleus. round deep purple cell.  round deep purple nucleus. \
            '+all_queries[0]
    #oval red cell. elliptical red cell.
    # 'round purple nucleus . purple cell . black dots . oval purple cell . round purple cell . deep purple cell . \
    #                 black circles .  \
    #                 purple nucleus . long elliptical purple cell . some purple dark elliptical body . red cell with ambiguous boundary '
    '''
    ...
    for i, batch in enumerate(_iterator):
        # if i>=40:#caution: wuyongjian: debug mode, should delete!!!!!!!!!!!!!!!!!!!!!!!!
        #     break
        ...
        if cfg.IMPROMPT.gvl==1:
            imprompt=None
        elif cfg.IMPROMPT.gvl == -1:
            SUPPORT_TARGET=targets
        else:
            SUPPORT_TARGET = None
        all_output = []
        mdetr_style_output = []
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if cfg.IMPROMPT.gvl==1:#wuyongjian: debug code ,remember to delete
                        if imprompt is None:
                        ...
                        output = model(images, captions=captions, positive_map=positive_map_label_to_token, reference_map=resized_tensor)
                    elif cfg.IMPROMPT.gvl==-1:
                        if 'coco1_2017_val' in cfg.DATASETS.TEST:
                            captions=['round purple nuclei.round black nuclei.']
                            def regenerate_positive_map_from_captions(captions,positive_map):
                                positive_map.clear()
                                splited_captions=captions[0].split('.')
                                phrase_count = 0
                                THE_LAST_POSITIVE_PLACE=0
                                for id, a_class_name in enumerate(splited_captions):
                                    a_class_name = a_class_name.strip()
                                    positions_list_of_current_phrase = []
                                    phrase_count += 1
                                    tokens=a_class_name.split(' ')
                                    for a_token in tokens:
                                        THE_LAST_POSITIVE_PLACE+=1
                                        positions_list_of_current_phrase.append(THE_LAST_POSITIVE_PLACE)
                                    positive_map.update({phrase_count: positions_list_of_current_phrase})
                                return positive_map
                            positive_map_label_to_token=regenerate_positive_map_from_captions(captions,positive_map_label_to_token)
                        if cfg.IMPROMPT.embed_via_dyhead:
                            output = model(images, captions=captions, positive_map=positive_map_label_to_token,
                                           reference_map=SUPPORT_TARGET)
                        output = model(images, captions=captions, positive_map=positive_map_label_to_token,
                                       reference_map=SUPPORT_TARGET)
                    else:
                        if cfg.GT_MASK_ON:
                        ...
                        if cfg.MODEL.RPN_ONLY:
                            output = model(images, targets=targets,captions=captions, positive_map=positive_map_label_to_token,idx=i)
                        else:
                            output = model(images)
                    output = [o.to(cpu_device) for o in output]

                    # if cfg.USE_CAM and i <= cfg.MAX_IMAGE_NUM_WHEN_USE_CAM:
                    ...
                    #     IMG_path = val_data_loader.dataset.root + '/' + \
                    #                val_data_loader.dataset.coco.imgs[id_to_img[image_ids[0]]]['file_name']
                    ...
                    #         continue
                    #     from skimage import io, transform
                    #     if 'fanew' in cfg.OUTPUT_DIR:
                    #         savpath = 'PLOT2/' + cfg.MODEL.WEIGHT.replace('/', '_').replace('.', '_')
                    #     else:
                    #         savpath = cfg.OUTPUT_DIR + '/cam_of_iter{}'.format(iteration)
                    #     if not os.path.exists(savpath):
                    #         os.mkdir(savpath)
                    #     io.imsave(
                    #         savpath + '/' + val_data_loader.dataset.coco.imgs[id_to_img[image_ids[0]]]['file_name'],
                    #         cam_image)
                    #     ORIsavpath = 'ORI_WITH_BOX'
                    #     if not os.path.exists(ORIsavpath):
                    #         os.mkdir(ORIsavpath)
                    #     ORIIM_PATH = ORIsavpath + '/' + val_data_loader.dataset.coco.imgs[id_to_img[image_ids[0]]][
                    #         'file_name']
                    ...
                    #         boxes = [ann['bbox'] for ann in dataset['annotations'] if
                    ...
                    if "flickr" in cfg.DATASETS.TEST[0]:
                        ...
                        all_output.append(output)
        if (visualizer is not None and not cfg.SOLVER.TEST_WITH_INFERENCE) or cfg.VIS_PRED_ONLY:
            ...
            no_background = True
            label_list = []
            for index, ii in enumerate(categories):
                if not no_background or (ii["name"] != "__background__" and ii['id'] != 0):
                    label_list.append(ii["name"])
            visualizer.entities =  label_list
            ###result, _ = visualizer.visualize_with_predictions(
            result, top_predictions = visualizer.visualize_with_predictions(
                image,
                visualizer_input, 
                thresh=0.3
            )
            ...
            #     #box = str(box)
            #     box =  ','.join(map(str, box.ravel().tolist()))
            #     box = np.fromstring(box, sep=',')
            #     with open('./visualize_bbox/img_{:03d}.csv'.format(i), 'a') as f:
            #         writer =csv.writer(f)
            #         writer.writerow(box)
            #
            # ###
    ...
    return evaluate(dataset=dataset, predictions=predictions, output_folder=output_folder, cfg=cfg, **extra_args)
