...
def inference(
    ...
    # convert to a torch.device for efficiency
    print('successfully input prompt:{}!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'.format(prompt))
    try:
        device = torch.device(device)
    except:
        device = device
    ...
    all_queries[0]=prompt#+all_queries[0]#oval red cell. elliptical red cell.
    ...
    batchcounter=0
    import datetime
    label2col={'nucleu round' : 'r',
               'nucleu round2': 'y',
               }
    now = '_{}_'.format(datetime.datetime.now()).replace(' ','_')
    for i, batch in enumerate(_iterator):
        ...
        # import numpy as np
        # toshow=transform.resize(toshow,(250,250,3))
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if cfg.IMPROMPT.gvl==2:
                        import matplotlib.pyplot as plt
                        plt.imshow(images.tensors[0].permute(1, 2, 0).cpu());
                        bb = targets[0].bbox
                        for i in range(bb.shape[0])[:]:
                            bb2plot = bb[i, :]
                            COLORS = ['w','r', 'orange', 'y', 'g', 'light green', 'b', 'purple']
                            x1, y1, x2, y2 = bb2plot.cpu()
                            this_cls=targets[0].extra_fields['labels'][i]
                            plt.gca().add_patch(
                                plt.Rectangle(xy=(x1, y1), width=(x2 - x1), height=(y2 - y1), edgecolor=COLORS[this_cls],
        ...
        if visualizer is not None:
            ...
            categories = all_queries[0].split('.')
            #print('working')#dataset.coco.dataset["categories"]###No!!!!!!!!!!!!!!!!!!!
            # except:
            #     lvis = dataset.lvis
            ...
            result, top_predictions = visualizer.visualize_with_predictions(
                ...
                thresh=0,
                # select_box_id=-1
            ) ##wuyongjian edit: new optional param : colors = -1 / 255
            ### added 2023.1.11
            # import csv
            ...
            # plt.imshow(result)
            # plt.show()
            if not os.path.exists("./vis_text{}".format(now)):
                os.mkdir("./vis_text{}".format(now))
            imshow(result, "./vis_text{}/img_{}.jpg".format(now,i))
    ...
    # if dataset.prompt_len>80:
    #     dataset.prompt_len=len(categories)-80
    return evaluate(dataset=dataset, predictions=predictions, output_folder=output_folder, **extra_args)
