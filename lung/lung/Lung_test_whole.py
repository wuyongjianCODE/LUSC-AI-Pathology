    ...
    if args.command == "detect":
        ...
        for svsname in os.listdir(SVSDIR):
            if '.svs' in svsname:
                ...
                #     print('LINE done')
                IMGS_LABELS_ARRAY=np.array(IMGS_LABELS)
                for colorid in range(1,4):
                    contours = measure.find_contours(IMGS_LABELS_ARRAY, colorid-0.5)
                    for n, contour in enumerate(contours):
                        plt.plot(contour[:, 0]*(224)/downscale, contour[:, 1]*(224)/downscale, linewidth=1,color=SWITCH[colorid])
                plt.savefig('{}_whole_classify_CONTOUR.png'.format(svsname[:-4]))
                plt.show()
                print('check')
