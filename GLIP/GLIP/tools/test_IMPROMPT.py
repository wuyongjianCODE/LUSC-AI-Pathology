...
from maskrcnn_benchmark.engine.predictor_glip_wuap import GLIPDemo
import os
import functools
import io
import os
...
def split_image_into_patches(image, patch_size=224, stride=224):
    """
    将图像切割成多个patch
    :param image: PIL Image对象
    :param patch_size: 每个patch的大小
    :param stride: 切割步长（通常等于patch_size）
    :return: patches列表和每个patch的位置信息
    """
    # 转换为numpy数组
    ...
    # 计算需要填充的高度和宽度
    pad_h = (n_rows * stride + patch_size - h) if (h - patch_size) % stride != 0 else 0
    pad_w = (n_cols * stride + patch_size - w) if (w - patch_size) % stride != 0 else 0
    ...
    # 切割图像
    for i in range(n_rows + (1 if pad_h > 0 else 0)):
        for j in range(n_cols + (1 if pad_w > 0 else 0)):
            # 计算当前patch的起始位置
            top = i * stride
            left = j * stride
            ...
            # 确保不越界
            if bottom > padded_array.shape[0] or right > padded_array.shape[1]:
                continue

            # 提取patch
            patch = padded_array[top:bottom, left:right, :]
...
def main():
    ...
    parser.add_argument(
        "--prompt",
        default='tumor. stroma.',#'round purple nuclei.round black nuclei.round blue nuclei.circle purple nuclei.circle black nuclei.circle blue nuclei.oblong purple nuclei.oblong black nuclei.oblong blue nuclei.',#'circle purple nucleus.circle blue nucleus.circle black nucleus.round purple nucleus.round blue nucleus. round black nucleus.oblong purple nucleus.oblong blue nucleus.oblong black nucleus.',
        help="prompt",
    )
    ...
    # logger.info("\n" + collect_env_info())
    glipdemo=GLIPDemo(cfg,0.3,load_model=True)
    image_dir="/home/data/jy/GLIP/DATASET/Cell_density/"
    ...
    for img_path in image_paths:
        ...
        patches,pposit=split_image_into_patches(original_image,112,112)

        transform = T.Compose([
            T.Resize((800, 800)),  # GLIP 的标准尺寸
            T.ToTensor(),
            T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            # T.Lambda(lambda x: x.unsqueeze(0))  # 添加批次维度
        ])
        for patch in patches:
            image_tensor = transform(patch).to("cuda")
            # 2. 进行推理
            predictions = glipdemo.inference(image_tensor, "circle purple nucleus.")
            img = cv2.cvtColor(np.asarray(patch), cv2.COLOR_RGB2BGR)
            result, top_predictions=glipdemo.visualize_with_predictions(img,predictions)
            # 3. 存储结果
            DEBUG=True
            if DEBUG:
            ...
            print(f"Processed: {img_path} | Detections: {len(predictions)}")


    return results
    # for impath in os.listdir("/home/data/jy/GLIP/DATASET/Cell_density/"):

    # model = build_detection_model(cfg)5
