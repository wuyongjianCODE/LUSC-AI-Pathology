...
if __name__ == '__main__':
    ...
    if args.command == "detect":
        itern=6
        savp='jy_TOSHOW_MULTISCALE_{}'.format(itern)
        if not os.path.exists(savp):
            os.mkdir(savp)
        # MaskrcnnPath = 'LUNGBIGDATA_{}.h5'.format(itern)
        MaskrcnnPath ='CODE/lung/lung/weights/model_final.h5'
        model.load_weights(os.path.abspath(MaskrcnnPath), by_name=True)
        DETECT_SET=False
        ONLY_SHOW_ORI=False
        if DETECT_SET:
            ...
            print('%s: %.2f%%' % (model.metrics_names[1], score[1] * 100))  # metrics1
            print('%s: %.2f%%' % (model.metrics_names[2], score[2] * 100))  # metrics2
        SVSDIR = r'/data2/深圳分院35例/'  # '/data3/SCC58/incomp-20/'#/data1/wyj/M/datasets/data2/svs1/'#'/data1/wyj/M/datasets/data2/svs3conv/'
        ...
        for root, dirs, files in os.walk(SVSDIR):
            for svsname in files:
                id += 1
                if id < int(args.start):
                    continue
                if '20-01031-22023-06-30' not in svsname :#and '1069555-8' not in svsname:
                    continue
                if '.svs' in svsname:  # and not os.path.exists('TOSHOW2/{}_whole_classify.png'.format(svsname[:-4])):
                    SVSPATH = root + '/' + svsname
