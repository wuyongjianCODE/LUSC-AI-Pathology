...
if __name__ == '__main__':
    ...
    if args.command == "detect":
        itern=6
        savp='TOSHOW_MULTISCALE_beijingnew_{}3'.format(itern)
        ...
        if DETECT_SET:
            ...
            print('%s: %.2f%%' % (model.metrics_names[1], score[1] * 100))  # metrics1
            print('%s: %.2f%%' % (model.metrics_names[2], score[2] * 100))  # metrics2
        SVSDIR ='/data2/beijingnew/山西肿瘤-新切新染_svs/'#r'/data2/beijingnew/2-新辅助鳞癌术后LN评估-130例-359张-20240227_svs/'# r'/data3/kfb-new/'#r'/data3/SCC58/incomp-20/'#/data1/wyj/M/datasets/data2/svs1/'#'/data1/wyj/M/datasets/data2/svs3conv/'
        ...
        if USE_NORM:
            ...
            target = "/data3/dataL/LUNG_NEW_NP/3/1031733-12_x19264_y14336_.png"
            import cv2
            normalizer2.fit(cv2.imread(target))
        # for subdirname in os.listdir(SVSDIR)
        if SAVE_CSV:
            import csv
            npy_savp = savp+'_npy'
            if not os.path.exists(npy_savp):
                os.mkdir(npy_savp)
            f=open(npy_savp+'/scores.csv','w')
            writer=csv.writer(f)
            writer.writerow(('filename','r','b','g','mpr'))
            npy_to_save=[]
        for svsname in os.listdir(SVSDIR):
            ...
            #     continue
            if '.svs' in svsname:  # and not os.path.exists('TOSHOW2/{}_whole_classify.png'.format(svsname[:-4])):
                ...
                if BAD_PATCH == 0:
                    ...
                    plt.savefig('{}/{}_whole_classify_r{}_b{}_g{}_mpr{}.png'.format(savp, svsname[:-4], r, b, g, mpr))
                    if SAVE_CSV:
                        csv_new_row = (svsname[:-4], r, g, b, mpr)
                        writer.writerow(csv_new_row)
                        np.save(
                            '{}/{}_whole_classify_r{}_b{}_g{}_mpr{}.png'.format(npy_savp, svsname[:-4], r, b, g, mpr),
                            npy_to_save)
                    # plt.show()
