...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
                                       ...
                                       positive_map=None,cfg=None,stage_idx=None,
                                       ):
        ...
        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors \
            ...
            boxlist.add_field("scores", torch.sqrt(per_box_cls))
            if cfg is not None:
                if cfg.IMPROMPT.gvl>0 or cfg.plot_tsne or cfg.IMPROMPT.gvl2>0:
                    per_stage_of_WU=torch.ones([1000,])
                    per_stage_of_WU*=stage_idx
                    boxlist.add_field("loc", per_box_loc)
                    boxlist.add_field("stage", per_stage_of_WU)
            boxlist = boxlist.clip_to_image(remove_empty=False)
            boxlist = remove_small_boxes(boxlist, self.min_size)
            results.append(boxlist)
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        for idx, (b, c, a) in enumerate(zip(box_regression, centerness, anchors)):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(b, c, a, o, t, d, positive_map,cfg,idx)
            )
...
def convert_grounding_to_od_logits(logits, box_cls, positive_map, score_agg=None):
    ...
    # 256 -> 80, average for each class . logits: 10000*256 box_cls:10000*80 positive_map:like dict{1: [1, 2, 3], 2: [5, 6, 7], 3: [9, 10, 11], 4: [13, 14, 15], 5: [17, 18, 19], 6: [21, 22, 23], 7: [25, 26, 27, 28], 8: [30, 31, 32, 33], 9: [35, 36, 37, 38]})
    #wuyongjian:256 string tokens(like 1012 for _START_, token 5163 stand for "nucleus", token 102 for '.' , token 0 for remained _, these tokens total number 256);
    # 80 class,like 'round red nucleus.' is a class, but this class has 3 token ,maybe 257 2776 5163 102.
    # If a bbox ,it is 56% similar to 257(round),44% similar to red,50% to nucleus,then the mean score is 0.5 for the whole class. As result, we will see this score in output: proposal.bbox['extra']
