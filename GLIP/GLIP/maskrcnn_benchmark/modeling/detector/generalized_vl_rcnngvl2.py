...
def regenerate_positive_map(tokenizer_input, positive_map, appending=0):
    positive_map.clear()
    ...
    for id, t_num in enumerate(input_ids):
        ...
        if t_num == 101:  # 跳过CLS token
            ...
            if len(positions_list_of_current_phrase) > 0:
                ...
                if appending >= 1 and positions_list_of_current_phrase:
    ...
    def forward(self,
        ...
        if captions is not None:
            ...
            if not self.training and positive_map is not None:
                positive_map=regenerate_positive_map(tokenizer_input,positive_map,appending=self.cfg.IMPROMPT.gvl2)
        ...
        if self.cfg.IMPROMPT.gvl2:
            ...
            if not self.training:
                ...
                # 1.2 筛选置信度TOP5的实例
                top5_idx = np.argsort(scores)[-5:][::-1]  # 降序取前5个索引
                top5_scores = scores[top5_idx]
                top5_bboxes = bboxes[top5_idx]
                top5_labels = labels[top5_idx]
                feats = []
                for idx in top5_idx:
                    feats.append(fused_visual_features[int(np.round(stages[idx]))].reshape(1, 256, -1)[:, :,
                                 int(np.round(locs[idx]))].squeeze(0).detach())
                # # ===================== 2. 处理ImageList，还原原图 =====================
                ...
                #
                # # 绘制TOP5框+标签+置信度
                # for bbox, score, label in zip(top5_bboxes, top5_scores, top5_labels):
                #     x1, y1, x2, y2 = bbox
                #     # 绘制矩形框
        ...
        if self.cfg.IMPROMPT.gvl2>0 and not self.training:
            proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets,
                                                                         language_dict_features, positive_map,
                                                                         captions, swint_feature_c4, feats)
        else:
            proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets,
                                                                             language_dict_features, positive_map,
                                                                             captions, swint_feature_c4)
