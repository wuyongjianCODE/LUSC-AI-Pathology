...
class GeneralizedVLRCNN(nn.Module):
    ...
    def forward(self,
            ...
            if abs(self.cfg.IMPROMPT.gvl)==1:
                ...
                if self.cfg.IMPROMPT.gvl==-1:
                    if self.cfg.IMPROMPT.embed_768_to_lang>=0:
                        if self.cfg.IMPROMPT.map_module == 'mlp':
                            visual_embeding_fcto768=self.fcto768(visual_embeding_flatten.contiguous().view(-1,49*256))[0,:]
        ...
        if self.cfg.use_skipt:
            language_dict_features,visual_features,targets=select_classes(language_dict_features,visual_features,targets)
