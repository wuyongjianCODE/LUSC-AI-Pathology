...
def convert_mask_grounding_to_od_logits(logits, positive_map_label_to_token, num_classes):
    ...
    for label_j in positive_map_label_to_token:
        od_logits[:, label_j-1, :, :] = logits[:, torch.LongTensor(positive_map_label_to_token[label_j]), :, :].mean(1)
...
class MaskPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, positive_map_label_to_token=None):
        ...
        """
        if self.vl_version:
            mask_prob = convert_mask_grounding_to_od_logits(x, positive_map_label_to_token, 0)
        ...
        mask_prob = mask_prob[index, labels,:, :]#wuyongjian
...
class Masker(object):
    ...
    def forward_single_image(self, masks, boxes):
        ...
        ]#wuyongjian : here used to be a bug,mask[0] changed to mask ,because masks is [100,800,800] ,nomore[100,1,800,800]
        if len(res) > 0:
