...
class GLIPDemo(object):
    ...
    def visualize_with_predictions(self, 
            ...
            text_pixel = 1,
            text_offset = 2,
            text_offset_original = 2,
            color = -1, #wuyongjian edit: give 255 will always get white bboxes;give -1 will get random colors each class
            select_box_id=-1): #wuyongjian edit: -1 nothing change; select id range(100) to debug a specific box
        self.color = color
        height, width = original_image.shape[:-1]
        predictions = predictions.resize((width, height))
        #top_predictions = self._post_process(predictions, thresh)
        top_predictions=predictions#wuyongjian edit:remember to change back!!!!!!!!!!!!!!!!!!!!!
        result = original_image.copy()
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions, alpha=alpha, box_pixel=box_pixel,select_box_id=select_box_id)
    ...
    def overlay_boxes(self, image, predictions, alpha=0.5, box_pixel = 3,select_box_id=-1):
        ...
        if select_box_id<0:
            for box, color,label in zip(boxes, colors,labels_list):
                box = box.to(torch.int64)
                top_left, bottom_right = box[:2].tolist(), box[2:].tolist()
                new_image = cv2.rectangle(
                    new_image, tuple(top_left), tuple(bottom_right), tuple(color), box_pixel)
        else:
            box = boxes[select_box_id].to(torch.int64)
            color=colors[select_box_id]
            top_left, bottom_right = box[:2].tolist(), box[2:].tolist()
            new_image = cv2.rectangle(
                new_image, tuple(top_left), tuple(bottom_right), tuple(color), box_pixel)
        ...
        for box, score, label , color in zip(boxes, scores, new_labels, colors):
            ...
            for x_prev, y_prev in previous_locations:
                if abs(x - x_prev) < abs(text_offset) and abs(y - y_prev) < abs(text_offset):
                    USE_THIS_TEXT = False
                    #y -= text_offset
            if label not in self.entities[:-80]:
                USE_THIS_TEXT = False
            if not USE_THIS_TEXT:
                continue
            cv2.putText(
